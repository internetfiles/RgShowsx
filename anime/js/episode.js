...
const ProxyApi = "https://corsproxy.notowner.workers.dev/?u=";
...
const AvailableServers = ["https://api.notowner.workers.dev"];
...
function selectServer(btn, sandbox = false) {
    ...
    const iframe = document.getElementById("RgShowsAnimeFrame");
    ...
    for (let i = 0; i < total.length; i++) {
        ...
        if (episodeId == EpisodeID) {
            if (isShortNumber(epNum)) {
                ephtml += `<div class="ep-slide ep-slider-playing"><a href="./episode.html?anime_id=${AnimeID}&episode_id=${episodeId}"><img onerror="retryImageLoad(this)" class="lzy_img" src="./static/loading1.gif" data-src=https://thumbjs.notowner.workers.dev/thumb/${episodeId}><div class=ep-title><span>Episode ${epNum} - Playing</span></div></a></div>`;
            } else {
                ephtml += `<div class="ep-slide ep-slider-playing"><a href="./episode.html?anime_id=${AnimeID}&episode_id=${episodeId}"><img onerror="retryImageLoad(this)" class="lzy_img" src="./static/loading1.gif" data-src=https://thumbjs.notowner.workers.dev/thumb/${episodeId}><div class=ep-title><span>Ep ${epNum} - Playing</span></div></a></div>`;
            }
        } else {
            if (isShortNumber(epNum)) {
                ...
                ephtml += `<div class=ep-slide><a href="./episode.html?anime_id=${AnimeID}&episode_id=${episodeId}"><img onerror="retryImageLoad(this)" class="lzy_img" src="./static/loading1.gif" data-src=https://thumbjs.notowner.workers.dev/thumb/${episodeId}><div class=ep-title><span>Ep ${epNum}</span></div></a></div>`;
            }
        }
    }
...
async function loadData() {
    ...
    document.getElementById("RgShowsAnimeFrame").focus();
}
